    ...
    func filmlerAl(kategori_id:Int) -> [Filmler] {
        ...
        do {
            ...
            while rs.next() {
                ...
                let film = Filmler(film_id: Int(rs.string(forColumn: "film_id"))!, film_ad: rs.string(forColumn: "film_ad")!, film_resim: rs.string(forColumn: "film_resim")!, film_yil: Int(rs.string(forColumn: "film_yil"))!, kategori: kategori, yonetmen: yonetmen)
                liste.append(film)
            }
        } catch  {
            print(error.localizedDescription)
